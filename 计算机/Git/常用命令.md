...
git remote  列出你指定的每一个远程仓库地址的简写，origin - 这是 Git 给你克隆的仓库服务器的默认名字

git remote -v  远程仓库地址使用的Git保存的简写与其对应的 URL

git remote add 简写名 服务器地址  添加一个新的远程仓库地址，同时指定一个你可以轻松引用的简写

git remote show 简写名  查看某一个远程仓库地址的更多信息，会列出远程仓库的 URL 与跟踪分支的信息

git remote rename 老简写名 新简写名  修改一个远程仓库地址的简写名

git remote rm 简写名

git remote prune 简写名  删除本地仓库遗留的远程仓库中已删除的分支

git remote add 简写名 远程仓库地址 添加远程仓库
...
https://www.cnblogs.com/wumz/p/9820314.html
